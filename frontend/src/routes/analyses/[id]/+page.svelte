...
<script lang="ts">
    ...
    let selectAnalysis = (pk: number, category: string, name: string) => {
        ...
        tabSet = 0;
        selectedAnalysisHistory = data.historyData.find((r) => r.category.name === category && r.name === name);
...
<div class="flex h-full border-t border-gray-300">
	<div class="flex-1 flex items-center justify-center border-r border-gray-300 overflow-x-hidden overflow-y-scroll h-[90vh] pl-3 pr-2">
                        ...
                        {#each table.rows as row}
                            <tr>
                                ...
                                <td class="text-center my-auto justify-center">
                                    <button onclick={() => selectAnalysis(row.pk, row.category.name, row.name)} class="btn-icon bg-green-500 [&>*]:pointer-events-none">
                                        <Play size={24} />
                                    </button>
                                </td>
                            </tr>
			...
			<svelte:fragment slot="panel">
				{#if tabSet === 0}
					<div class="flex items-center w-full flex-col gap-2 overflow-y-scroll h-[84vh]">
                        ...
                        {#if selectedAnalysis}
                                <form class="flex flex-col gap-2 w-[30vw]" method="POST" action="?/edit" use:enhance onsubmit={() => window.location.reload()}>
                                <input type="hidden" name="pk" value={selectedAnalysis.pk} />
                                <input type="hidden" name="viewas_user" value={data.viewAsUser} />
                                <label class="label">
                                    <span>Result</span>
                                    <input name="result" class="input" type="text" placeholder="Result" value={selectedAnalysis.result} />
                                </label>
                                <label class="label">
                                    <span>Measurement Unit</span>
                                    <input name="measurement_unit" class="input" type="text" placeholder="Measurement Unit" value={selectedAnalysis.measurement_unit} />
                                </label>
                                <label class="label">
                                    <span>Minimum Range</span>
                                    <input name="range_min" class="input" type="number" placeholder="Minimum Range" value={selectedAnalysis.range_min} />
                                </label>
                                <label class="label">
                                    <span>Minimum Range</span>
                                    <input name="range_max" class="input" type="number" placeholder="Maximum Range" value={selectedAnalysis.range_max} />
                                </label>
                                <label class="label">
                                    <input name="in_range" class="checkbox" type="checkbox" placeholder="In Range" checked={selectedAnalysis.in_range} />
                                    <span>In Range</span>
                                </label>
                                <label class="label">
                                    <span>AI Suggestion</span>
                                    <textarea name="suggestion" class="textarea h-40 block" placeholder="AI Suggestion" disabled value={selectedAnalysis.suggestion} ></textarea>
                                </label>
                                <label class="label">
                                    <span>Doctor Note</span>
                                    <textarea name="doctor_note" class="textarea h-40 block" placeholder="Doctor Note" value={selectedAnalysis.doctor_note} disabled={!data.user?.is_doctor}></textarea>
                                </label>
                                <button class="btn btn-md bg-green-500" type="submit">Save</button>
                            </form>
                            <form class="flex flex-col gap-2 w-[30vw]" method="POST" action="?/regenerate" use:enhance onsubmit={() => window.location.reload()}>
                                <input type="hidden" name="pk" value={selectedAnalysis.pk} />
                                <input type="hidden" name="viewas_user" value={data.viewAsUser} />
                                <button class="btn btn-md bg-green-500" type="submit">Regenerate AI Suggestion</button>
                            </form>
                        {/if}
					</div>
				{:else if tabSet === 1}
                                ...
                                <AccordionItem>
                                    ...
                                    <svelte:fragment slot="content">
                                        {#if result.suggestion && result.doctor_note}
                                            <b>AI Suggestion:</b> {result.suggestion} <br/>
