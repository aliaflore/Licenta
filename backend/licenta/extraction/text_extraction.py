...
SYSTEM_TEXT = """
    ...
    Inside range_min and range_max print only numbers, no other characters or texts. If there is not range minimum or maximum, set it to null. The result should be a number or the booleans "false" or "true".
    ...
    analysis found in the given text. Translate measurement_unit in English. Do not translate the analysis name. Also put the date of the analysis in the "date" key from the parent object, in ISO-8601 format.
    """
...
def createJSON(file):
    completion = settings.OPENAPI_CLIENT.chat.completions.create(
        model="gpt-4-turbo",
        messages=[
            {
                "role": "system",
